...
test('ConversationRouter: primeiro contato misto responde dúvida antes de pedir nome', () => {
  ...
  assert.doesNotMatch(out.resposta_cliente, /Me chamo Assistente|muito prazer/);
  assert.match(out.resposta_cliente, /O valor depende/);
...
test('ConversationRouter: multi-info de tattoo persiste campos e pede só foto', () => {
  const out = routeConversationTurn({
    estado_atual: 'tattoo',
  ...
  assert.equal(out.dados_persistidos.local_corpo, 'antebraço');
...
test('ConversationRouter: recupera multi-info em resposta ao local pendente', () => {
  const out = routeConversationTurn({
    estado_atual: 'tattoo',
    mensagem: 'na perna, tenho 1,81',
    conversa: { dados_coletados: { descricao_curta: 'rosa', estilo: 'fineline' }, dados_cadastro: {} },
    historico: [
  ...
  assert.deepEqual(out.campos_faltando, []);
  assert.match(out.resposta_cliente, /foto do local/i);
  assert.doesNotMatch(out.resposta_cliente, /qual tua altura|parte do corpo|qual estilo/i);
});
...
test('ConversationRouter: menoridade explicita sem numero aciona humano sem inventar data', () => {
  ...
  assert.equal(out.proxima_acao, 'erro');
  assert.equal(out.escalation.reason_code, 'minor_age');
  assert.equal(out.minor_age_resolution.age, null);
  assert.deepEqual(out.dados_persistidos, {});
  assert.match(out.resposta_cliente, /menos de 18 anos/i);
  assert.match(out.resposta_cliente, /respons[aá]vel legal/i);
});

test('ConversationRouter: autorizacao de responsavel aciona humano sem inventar data', () => {
  const out = routeConversationTurn({
    estado_atual: 'cadastro',
    mensagem: 'minha mãe autorizou',
    conversa: { dados_coletados: {}, dados_cadastro: { nome: 'Joao Silva' } },
    historico: [
      { role: 'assistant', content: 'Me passa tua data de nascimento completa?' },
    ],
  ...
  assert.equal(out.intent, 'minor_age_explicit');
  assert.equal(out.reason, 'guardian_consent_minor_age_signal');
  assert.equal(out.estado_novo, 'aguardando_tatuador');
  assert.equal(out.proxima_acao, 'erro');
  assert.equal(out.escalation.reason_code, 'minor_age');
  assert.equal(out.minor_age_resolution.age, null);
...
test('ConversationRouter: autorizacao dos pais aciona humano sem inventar data', () => {
  const out = routeConversationTurn({
    estado_atual: 'cadastro',
    mensagem: 'tenho autorização dos meus pais',
    conversa: { dados_coletados: {}, dados_cadastro: { nome: 'Joao Silva' } },
    historico: [
      { role: 'assistant', content: 'Me passa tua data de nascimento completa?' },
    ],
...
test('ConversationRouter: recusa natural de email por canal atual resolve email pendente', () => {
  const out = routeConversationTurn({
    estado_atual: 'cadastro',
...
test('ConversationRouter: recusa natural "por aqui mesmo" resolve email pendente', () => {
  const out = routeConversationTurn({
    estado_atual: 'cadastro',
    mensagem: 'por aqui mesmo',
    conversa: {
      dados_coletados: {},
      dados_cadastro: { nome: 'Joao Silva', data_nascimento: '1995-03-12' },
    },
