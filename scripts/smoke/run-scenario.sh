...
setup_tenant_style_ask_artist() {
  ...
  body="$(jq -c '{
    config_agente: ((.config_agente // {}) + {
      ...
      bloqueia_estilos_fora_catalogo:false
    })
  }' "$restore_file")"
  supa_patch "tenants?id=eq.${TENANT_ID}" "$body" >/dev/null
  echo "tenant setup ok: config_agente.out_of_catalog_behavior=ask_artist tenant=$TENANT_ID"
}
...
run_setup() {
  case "$SETUP" in
    ...
    tenant_style_ask_artist) setup_tenant_style_ask_artist ;;
