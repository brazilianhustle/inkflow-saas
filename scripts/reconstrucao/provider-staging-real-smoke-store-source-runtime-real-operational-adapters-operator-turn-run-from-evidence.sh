...
export INKFLOW_PLATFORM_DIR="$PLATFORM_DIR"
export INKFLOW_ENV="${INKFLOW_ENV:-local}"
export PROVIDER_ENV="${PROVIDER_ENV:-local}"

node "$SCRIPT_DIR/provider-staging-real-smoke-store-source-runtime-real-operational-adapters-operator-turn-run-from-evidence.mjs" "$@"
