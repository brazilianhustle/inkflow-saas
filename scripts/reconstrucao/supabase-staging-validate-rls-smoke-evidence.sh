...
if [ "$#" -lt 1 ]; then
  echo "Uso: npm run supabase:staging:validate-rls-smoke-evidence -- docs/evidence/supabase-staging/<record>.md" >&2
  exit 1
fi
...
cd "$PLATFORM_DIR"
npm run supabase:staging:validate-rls-smoke-evidence -- "$@"
