...
Proximo passo seguro: construir `prepare_provider_staging_real_transport_ports_harness`, ainda sem trafego real. Aprovacao humana `APPROVE_PROVIDER_STAGING_REAL_SMOKE_EXECUTION` foi recebida; a tentativa real anterior parou corretamente em `injected_driver_required`; o checkpoint `provider:staging:real-transport-runner-injection` provou o caminho de injecao; e o checkpoint `provider:staging:real-transport-ports` criou as portas provider-aware aprovadas com default bloqueado, dry-run redigido, `provider_staging_real_transport_ports_ready=true`, `provider_staging_real_provider_traffic_authorized=false`, `provider_staging_smoke_executed=false`, `connects_to_provider=false`, `executable_provider_commands=false` e `next_checkpoint=prepare_provider_staging_real_transport_ports_harness`.
