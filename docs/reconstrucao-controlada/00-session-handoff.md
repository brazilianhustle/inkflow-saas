...
```text
05d5765 feat: add provider real smoke gate review
```
...
Proximo passo seguro: decisao humana para autorizar ou nao o primeiro Provider staging real smoke. O gate review local-only passou com `ready_for_human_provider_staging_smoke_decision=true`, `human_decision_required=true`, `provider_staging_smoke_execution_authorized=false`, `provider_staging_smoke_executed=false`, `connects_to_provider=false`, `executable_provider_commands=false` e `next_checkpoint=operator_decides_provider_staging_real_smoke_execution`. A frase de aprovacao futura, se for executar o smoke real, e `APPROVE_PROVIDER_STAGING_REAL_SMOKE_EXECUTION`.
