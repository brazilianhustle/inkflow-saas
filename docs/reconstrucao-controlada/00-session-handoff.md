...
```text
a0d03a2 feat: add provider real transport runner skeleton
```
...
Proximo passo seguro: preparar o Provider staging real transport runner harness. O skeleton do transport runner real passou em modo local-only, com portas injetaveis, default bloqueado, `provider_staging_transport_runner_ready=false`, `provider_staging_smoke_execution_authorized=false`, `provider_staging_smoke_executed=false`, `provider_staging_smoke_evidence_captured=false`, `connects_to_provider=false`, `executable_provider_commands=false` e `next_checkpoint=prepare_provider_staging_real_transport_runner_harness`. A execucao real continua bloqueada ate existir harness com portas simuladas + runner real injetado + evidencia operacional validada.
