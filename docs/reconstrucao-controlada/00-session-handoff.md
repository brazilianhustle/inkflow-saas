...
```text
956d5bd feat(provider): add real transport runtime binding harness
```
...
Proximo passo seguro: construir `prepare_provider_staging_real_transport_runtime_binding_executor_integration`, ainda sem trafego real. Aprovacao humana `APPROVE_PROVIDER_STAGING_REAL_SMOKE_EXECUTION` foi recebida; a tentativa real anterior parou corretamente em `injected_driver_required`; `provider:staging:real-transport-runner-injection` provou o caminho de injecao; `provider:staging:real-transport-ports` criou as portas provider-aware aprovadas; `provider:staging:real-transport-ports-harness` provou boundary coverage e ordem; `provider:staging:real-transport-client-adapters` preparou a superficie de client adapters; `provider:staging:real-transport-client-adapter-harness` provou adapters instrumentados com uma invocacao por adapter, adapter certo por etapa, rollback antes de evidencia e dry-run in-memory; `provider:staging:real-transport-runtime-binding-plan` mapeou os 8 adapters para boundaries worker/server-only; `provider:staging:real-transport-runtime-binding-skeleton` criou bindings runtime injetaveis e bloqueou contextos `browser`, `admin-ui`, `static-page`, `documentation` e `test-fixture`; e `provider:staging:real-transport-runtime-binding-harness` provou o caminho skeleton -> real transport runner skeleton -> smoke executor em `simulation=true`, com evidencia apenas em memoria, mantendo `provider_staging_real_provider_traffic_authorized=false`, `provider_staging_smoke_executed=false`, `connects_to_provider=false`, `executable_provider_commands=false`, `in_memory_evidence_validated=true`, `simulated_evidence_written=false` e `next_checkpoint=prepare_provider_staging_real_transport_runtime_binding_executor_integration`.
