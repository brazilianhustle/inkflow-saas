...
```text
0c2a2b3 feat(provider): add real transport ports harness
```
...
Proximo passo seguro: construir `prepare_provider_staging_real_transport_client_adapters`, ainda sem trafego real. Aprovacao humana `APPROVE_PROVIDER_STAGING_REAL_SMOKE_EXECUTION` foi recebida; a tentativa real anterior parou corretamente em `injected_driver_required`; o checkpoint `provider:staging:real-transport-runner-injection` provou o caminho de injecao; o checkpoint `provider:staging:real-transport-ports` criou as portas provider-aware aprovadas com default bloqueado; e o checkpoint `provider:staging:real-transport-ports-harness` provou boundary coverage, port mapping, ordem de sequencia, rollback antes de evidencia e dry-run in-memory, com `provider_staging_real_provider_traffic_authorized=false`, `provider_staging_smoke_executed=false`, `connects_to_provider=false`, `executable_provider_commands=false`, `dry_run_harness_executed=true`, `dry_run_evidence_written=false` e `next_checkpoint=prepare_provider_staging_real_transport_client_adapters`.
