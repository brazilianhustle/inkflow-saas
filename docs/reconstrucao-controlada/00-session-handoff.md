...
```text
77136a1 feat(provider): add real transport runtime binding plan
```
...
Proximo passo seguro: construir `build_provider_staging_real_transport_runtime_binding_skeleton`, ainda sem trafego real. Aprovacao humana `APPROVE_PROVIDER_STAGING_REAL_SMOKE_EXECUTION` foi recebida; a tentativa real anterior parou corretamente em `injected_driver_required`; `provider:staging:real-transport-runner-injection` provou o caminho de injecao; `provider:staging:real-transport-ports` criou as portas provider-aware aprovadas; `provider:staging:real-transport-ports-harness` provou boundary coverage e ordem; `provider:staging:real-transport-client-adapters` preparou a superficie de client adapters; `provider:staging:real-transport-client-adapter-harness` provou adapters instrumentados com uma invocacao por adapter, adapter certo por etapa, rollback antes de evidencia e dry-run in-memory; e `provider:staging:real-transport-runtime-binding-plan` mapeou os 8 adapters para boundaries worker/server-only com contextos proibidos `browser`, `admin-ui`, `static-page`, `documentation` e `test-fixture`, mantendo `provider_staging_real_provider_traffic_authorized=false`, `provider_staging_smoke_executed=false`, `connects_to_provider=false`, `executable_provider_commands=false`, `redacts_provider_handles=true` e `next_checkpoint=build_provider_staging_real_transport_runtime_binding_skeleton`.
