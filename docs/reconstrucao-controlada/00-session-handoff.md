...
```text
79dcd9d feat: add provider driver binding skeleton
```
...
Proximo passo seguro: preparar o harness dos driver bindings de staging, ainda sem provider real. O skeleton passou com os seis drivers do adapter e seis boundaries injetaveis, default bloqueado, prova redigida, `provider_staging_driver_binding_execution_default=false`, `provider_staging_smoke_executed=false`, `connects_to_provider=false`, `executable_provider_commands=false` e `next_checkpoint=prepare_provider_staging_driver_binding_harness`.
