...
```text
ac303ad feat: add provider driver binding harness
```
...
Proximo passo seguro: preparar o plano de runtime resolution dos drivers de staging, ainda sem provider real. O harness passou com simulacao local pelo skeleton -> adapter -> executor, `harness_simulated_run_executed=true`, `provider_staging_smoke_executed=false`, `provider_staging_smoke_evidence_captured=false`, `connects_to_provider=false`, `executable_provider_commands=false`, `simulated_evidence_written=false` e `next_checkpoint=prepare_provider_staging_driver_runtime_resolution_plan`.
