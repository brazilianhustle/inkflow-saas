...
```text
da32434 feat(provider): add real transport client adapters
```
...
Proximo passo seguro: construir `prepare_provider_staging_real_transport_client_adapter_harness`, ainda sem trafego real. Aprovacao humana `APPROVE_PROVIDER_STAGING_REAL_SMOKE_EXECUTION` foi recebida; a tentativa real anterior parou corretamente em `injected_driver_required`; o checkpoint `provider:staging:real-transport-runner-injection` provou o caminho de injecao; `provider:staging:real-transport-ports` criou as portas provider-aware aprovadas com default bloqueado; `provider:staging:real-transport-ports-harness` provou boundary coverage e ordem; e `provider:staging:real-transport-client-adapters` preparou a superficie de client adapters, com default bloqueado, mapeamento adapter->boundary, dry-run redigido, `provider_staging_real_provider_traffic_authorized=false`, `provider_staging_smoke_executed=false`, `connects_to_provider=false`, `executable_provider_commands=false`, `dry_run_client_adapters_validated=true`, `dry_run_boundary_mapping_validated=true`, `dry_run_harness_executed=true`, `dry_run_evidence_written=false` e `next_checkpoint=prepare_provider_staging_real_transport_client_adapter_harness`.
