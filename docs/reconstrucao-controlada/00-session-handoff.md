...
Proximo passo seguro: decisao humana explicita sobre a tentativa real controlada de Provider staging smoke. O gate final foi revisado apos a integracao do transport runner e esta verde, com `ready_for_human_provider_staging_smoke_decision=true`, `provider_staging_real_transport_runner_integrated=true`, `provider_staging_smoke_execution_authorized=false`, `provider_staging_smoke_executed=false`, `provider_staging_smoke_evidence_captured=false`, `connects_to_provider=false`, `executable_provider_commands=false`, `human_decision_required=true` e `required_approval_phrase=APPROVE_PROVIDER_STAGING_REAL_SMOKE_EXECUTION`. A execucao real continua bloqueada ate nova decisao humana e runner real injetado.
