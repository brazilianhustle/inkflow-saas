...
```text
5bd4c5d feat: add provider runtime resolution plan
```
...
Proximo passo seguro: construir o skeleton de runtime resolution dos drivers de staging, ainda sem provider real. O plano de runtime resolution passou em modo local-only, amarrado ao harness anterior, com `provider_staging_driver_binding_harness_ready=true`, `provider_staging_runtime_resolution_execution_default=false`, `provider_staging_smoke_executed=false`, `provider_staging_smoke_evidence_captured=false`, `connects_to_provider=false`, `executable_provider_commands=false`, `redacts_provider_handles=true` e `next_checkpoint=build_provider_staging_driver_runtime_resolution_skeleton`.
