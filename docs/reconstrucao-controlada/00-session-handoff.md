...
- Ordem segura atual: `provider:staging:build-roundtrip-package -- --evidence-dir .smoke-evidence/<run> --init-source` -> operador preenche fonte redigida -> `provider:staging:review-roundtrip-source -- --evidence-dir .smoke-evidence/<run>` -> `provider:staging:build-roundtrip-package -- --evidence-dir .smoke-evidence/<run> --write` -> `provider:staging:real-smoke-store-source-runtime-real-operational-adapters-operator-turn-run-from-evidence -- --execute ...` -> evidence review -> wave close.
- Validacoes apos auditor de fonte: `node --test tests/reconstrucao/provider-staging-build-roundtrip-package.test.mjs tests/reconstrucao/provider-staging-review-roundtrip-source.test.mjs tests/reconstrucao/provider-staging-operator-turn-run-from-evidence.test.mjs` PASS 18/18; `npm run provider:staging:review-roundtrip-source` PASS bloqueado com `next_checkpoint=provide_smoke_evidence_dir_or_provider_roundtrip_source`; `npm test` PASS 1275/1275.
