...
Status: novo repo `inkflow-platform` criado localmente com contratos funcionais isolados, `services/bot-orchestrator`, adapters simulados, entrega simulada outbox->receipt, audit store local integrado e `packages/persistence-contracts`, sem canais reais, sem painel, sem secrets e sem deploy.
...
Proxima decisao: checkpoint para iniciar skeleton de `apps/admin`, usando os contratos de persistencia como base. Nao iniciar adapter real de WhatsApp/Supabase/Telegram nem Supabase real sem checkpoint explicito.
