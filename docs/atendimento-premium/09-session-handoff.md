...
Segunda onda Level 4A declarada: `level4a-wave-2-cadastro-question-policy`, risco amarelo. Objetivo: fortalecer `QuestionPolicy` para respostas de cadastro pendente. Micro-slice atual: `cadastro-question-policy-nome`. Cada micro-slice conversacional exige teste local relevante, HTTP radar, WhatsApp real definitivo, CI/deploy PASS e registro de evidencia.
