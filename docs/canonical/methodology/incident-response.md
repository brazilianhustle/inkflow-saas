...
- **vps-limits** ✅ implementado (PR <PR-AUDITOR-VPS-LIMITS> — `<merge-sha>`): detecta saturação de recursos no VPS — RAM/Disk/Load avg/Egress mensal via endpoint health. Routine Anthropic a cada 6h (offset +15min). Alerta `[warn/critical] [vps-limits]` → investigar manualmente via SSH (runbook dedicado pendente). Doc canônico: [auditores.md#vps-limits](../auditores.md#vps-limits).
