...
- **vps-limits** ✅ em prod 2026-04-30 (PR #14 — `8095fc7` + pivot cron-worker pós-merge): detecta saturação de recursos no VPS — RAM/Disk/Load avg/Egress mensal via endpoint health. cron-worker `inkflow-cron` dispatcher `15 */6 * * *` UTC (pivot de Routine — CCR allowlist 403 bloqueio). Alerta `[warn/critical] [vps-limits]` → investigar manualmente via SSH (runbook dedicado pendente). Doc canônico: [auditores.md#vps-limits](../auditores.md#vps-limits).
- **rls-drift** ✅ em prod 2026-04-30 (PR #16 — `02a3bae`): detecta drift de segurança Supabase via SQL queries diretas — tables sem RLS (não allowlisted) → warn, functions sem search_path → critical. Routine Anthropic primary 24h (07:00 UTC) com reasoning Claude + cron-worker pivot pré-configurado. **Deviation crítico cravado**: spec §5.4 cravou advisor REST API que não existe; pivotamos pra SQL via `/database/query`. Alerta `[warn/critical] [rls-drift]` → narrative no payload contextualiza com PR/commit relacionado. Doc canônico: [auditores.md#rls-drift](../auditores.md#rls-drift). **Sub-projeto 3 fechado 5/5 ✅**.
