...
// Pausa o bot pra uma conversa específica via UI manual (botão "Assumir" no Painel Conversas).
// Body: { conversa_id, studio_token }
// Auth: studio_token HMAC v1 (ou legacy UUID via verifyStudioTokenOrLegacy).
...
export async function onRequest(context) {
  ...
  // Fetch conversa pra pegar estado_atual + verificar tenancy
  ...
  if (!upd.ok) {
    ...
    console.error('assumir: PATCH conversa falhou', upd.status, errText);
